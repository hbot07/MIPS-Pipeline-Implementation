...
struct MIPS_Architecture {
    ...
    std::unordered_map<std::string, std::function<int(MIPS_Architecture &, std::string, std::string,
    ...
    MIPS_Architecture(std::ifstream &file) {
        instructions = {{"add",  &MIPS_Architecture::add},
                        {"sub",  &MIPS_Architecture::sub},
                        {"mul",  &MIPS_Architecture::mul},
                        {"beq",  &MIPS_Architecture::beq},
                        {"bne",  &MIPS_Architecture::bne},
                        {"slt",  &MIPS_Architecture::slt},
                        {"j",    &MIPS_Architecture::j},
    ...
    inline bool checkLabel(std::string str) {
        return str.size() > 0 && isalpha(str[0]) &&
               all_of(++str.begin(), str.end(), [](char c) { return (bool) isalnum(c); }) &&
    ...
    bool checkRegisters(std::vector<std::string> regs) {
        return std::all_of(regs.begin(), regs.end(), [&](std::string r) { return checkRegister(r); });
    }
    ...
    */
    ...
    void executeCommandsUnpipelined() {
        ...
        while (PCcurr < commands.size()) {
            ...
            if (rest_flag) {
                exit_code ret = (exit_code) instructions[command0buffer](*this, command1buffer, command2buffer,
                                                                         command3buffer);
                if (ret != SUCCESS) {
