...
struct MIPS_Architecture
{
	...
	*/
    ...
    std::vector<std::string> & instructionFetch(int PCcurr1) {
        return commands[PCcurr1];
    }
    ...
    void executeCommandsPipelined()
    {
        ...
        {
            //Stage 1
            std::vector<std::string> &command = instructionFetch(PCcurr);
            ...
            //Stage 2
            int function_code = instructionDecode(command);
            ++clockCycles;
            ++commandCount[PCcurr];

            //Stage 3
            int alu_result = aluExecute(registers[registerMap[command[2]]], registers[registerMap[command[3]]], 0, function_code);
            ++clockCycles;
            ++commandCount[PCcurr];
            ...
            //Stage 5
            registers[registerMap[command[1]]] = alu_result;
            ++clockCycles;
            ++commandCount[PCcurr];
